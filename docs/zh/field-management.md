...
找到文件`app/Admin/bootstrap.php`,如果文件不存在，请更新`daimakuai`，然后新建该文件
...
>类中的静态资源也同样可以从外部引入，参考[Editor.php](https://github.com/happyonion/daimakuai/blob/1.3/src/Form/Field/Editor.php)
...
最后找到文件`app/Admin/bootstrap.php`,如果文件不存在，请更新`daimakuai`，然后新建该文件,添加下面代码：
...
[wangEditor](http://www.wangeditor.com/)是一个优秀的国产的轻量级富文本编辑器，如果`daimakuai`自带的基于`ckeditor`的编辑器组件使用上有问题，可以通过下面的步骤可以集成它，并覆盖掉`ckeditor`：
...
然后注册进`daimakuai`,在`app/Admin/bootstrap.php`中添加以下代码：
...
> 组件类中指定了`admin::form.editor`作为视图文件，视图文件路径在`vendor/jblv/daimakuai/views/form/editor.blade.php`，如果需要修改视图文件，可以将上述视图文件拷贝到`resources/views`目录下自行修改，然后在组件类`app/Admin/Extensions/WangEditor.php`的`$view`属性指定刚才修改的view即可。
